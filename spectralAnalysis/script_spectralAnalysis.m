...
% Here we assume all PLYs are stored at the same spatial scale
pix2um = 0.302 ;  % um / pixel
...
% Here we note that we measure the radial position of the mesh relative to 
% a reference sphere and decompose that scalar field of surface topography 
    ...
    for ii = 1:length(fns)
        ...
        if ~exist(outfn, 'file') || overwrite 
            ...
            if ~exist(outfn, 'file') || overwrite 
                ...
                % Here we compute the radial position of the mesh relative
                % to the reference sphere's radial position (radii0). In
                % other words, signalType == 'radialu' ;
                ff =  radii - radii0 ;
...
clc
% Pre-allocate the cell arrays for mean, standard deviations (std), and 
% standard error on the mean (ste)
meanPowers = {} ;
stdPowers = {} ;
stePowers = {} ;

% consider each experimental case
hs = {} ;
