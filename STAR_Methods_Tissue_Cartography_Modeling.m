...
%Make sure that all the files from ImSAnE, this script, and your images are
...
xp.detector.inspectQuality(inspectOptions, xp.stack); %comment out this line if using Leica images
...
% Save the resolution of the pointcloud/mesh in microns per pixel
header = 'Isotropic resolution of the PLY mesh space in microns per pixel/voxel';
resFn = fullfile(projectDir, 'MYFILENAME resolution in microns per pixels.txt') ;
write_txt_with_header(resFn, fileMeta.stackResolution(1), header) ;
...
pause_button = "on. Change the file name in the second-to-last line and then press a key to continue"
